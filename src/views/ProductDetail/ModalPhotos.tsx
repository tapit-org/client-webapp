...
const ModalPhotos: FC<ModalPhotosProps> = ({
	...
	let MY_GLIDEJS = useMemo(() => {
		return new Glide(`.${UNIQUE_CLASS}`, {
			// @ts-ignore
			direction:
				document.querySelector('html')?.getAttribute('dir') === 'rtl' ? 'rtl' : 'ltr',
			gap: 10,
			perView: 1,
			startAt: initFocus,
		});
	...
	const renderSlider = () => {
		return (
			...
			>
				{/*  */}
				<div className="controls_nav glide__bullets mb-4" data-glide-el="controls[nav]">
					{imgs.map((_, index) => (
				...
				<div className="xl:absolute z-20 xl:-inset-x-20 max-w-6xl my-2 mx-auto top-full xl:top-1/2 transform xl:-translate-y-1/2 flex xl:justify-between glide__arrows">
					<NextPrev onlyPrev className="mr-1.5" btnClassName="w-8 h-8 sm:w-10 sm:h-10 " />
					<NextPrev onlyNext className="ml-1.5" btnClassName="w-8 h-8 sm:w-10 sm:h-10 " />
				</div>
			</div>
		);
	};
